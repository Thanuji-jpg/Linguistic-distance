"""
Depression-bipolar real-patient run (WHO Zone IV only).

Loads every unique HAC 0/1 disease profile, keeps those with
depressionbipolar = Present (all other comorbidities unchanged),
...
DISEASE_GROUP = "depressionbipolar"
NUM_ITERATIONS = 1
...
def load_depressionbipolar_patients(who):
    """
    Load all unique HAC profiles, then keep depressionbipolar = Present.
    """
    who.hac_profile_mode = "unique_profiles"
    who.load_hac_comorbidity_settings()

    if "depressionbipolar" not in who.COMORBIDITY_FIELD_NAMES:
        raise ValueError(
            "HAC comorbidity fields do not include depressionbipolar."
        )

    matched = [
        condition.copy()
        for condition in who.hac_loaded_conditions
        if condition["depressionbipolar"]
        == who.COMORBIDITY_PRESENT_LABEL
    ]

    if not matched:
        raise ValueError(
            "No unique HAC profiles have depressionbipolar = Present."
        )
...
def confirm_paid_api_calls(patient_count, model_name):
    ...
    print(
        f"\nReal unique HAC profiles with depressionbipolar = Present: "
        f"{patient_count}"
    )
    print(
...
def create_record(who, condition, audit_scenario, overall_request_number, prompt_sha256):
    ...
    return {
        ...
        **{
        ...
        "condition_mode": "hac_dataset_depressionbipolar_present",
...
def main():
    ...
    patients = load_depressionbipolar_patients(who)

    print("=" * 80)
    print("DEPRESSION BIPOLAR REAL-PATIENT RUN (ZONE IV FIXED)")
    print("=" * 80)
    print(
    ...
    print(
        f"Profiles with depressionbipolar = Present: "
        f"{len(patients)}"
    )
    print(
    ...
    output_csv_path = (
        OUTPUT_DIR
        / f"depressionbipolar_real_patients_{run_timestamp}_responses.csv"
    )
    ...
    print("=" * 80)
    print("DEPRESSION BIPOLAR RUN COMPLETE")
    print("=" * 80)
